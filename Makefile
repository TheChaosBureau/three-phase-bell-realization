...
SPICE_DRIVEN_ROBUSTNESS_TRIALS ?= 1200
SPICE_DRIVEN_ROBUSTNESS_SEED ?= 20260411
...
# ----------------------------------------------------------------------
# Foundations of Physics manuscript (paper/paper.qmd)
#
# `paper.qmd` is a pure-prose Quarto document with no code blocks, so it
# does NOT need --execute (and therefore does not need the poetry venv).
# Output lands at paper/paper.pdf and is also copied to artifacts/paper/.
# ----------------------------------------------------------------------
...
paper:
	quarto render paper/paper.qmd \
		...
		--metadata date="$(DATE)"
	mkdir -p artifacts/paper
	cp paper/paper.pdf artifacts/paper/paper.pdf
	@printf '\a'
	@echo "[paper] wrote paper/paper.pdf and artifacts/paper/paper.pdf"
...
paper-docx:
	quarto render paper/paper.qmd \
		...
		--metadata date="$(DATE)"
	mkdir -p artifacts/paper
	cp paper/paper.docx artifacts/paper/paper.docx
	@printf '\a'
	@echo "[paper-docx] wrote paper/paper.docx and artifacts/paper/paper.docx"
...
spice-driven-robustness-report:
	...
	poetry run $(PYTHON) -m physical_front_end_candidate.experiments.build_spice_driven_robustness_report \
		...
		--seed "$(SPICE_DRIVEN_ROBUSTNESS_SEED)"
	@printf '\a'

spice-driven-baseline-reconciliation-report:
	mkdir -p "$(SPICE_DRIVEN_BASELINE_RECONCILIATION_OUTDIR)"
	poetry run $(PYTHON) -m physical_front_end_candidate.experiments.build_spice_driven_baseline_reconciliation_report \
		--outdir "$(SPICE_DRIVEN_BASELINE_RECONCILIATION_OUTDIR)" \
		--detector-next-summary-csv "$(SPICE_DRIVEN_BASELINE_RECONCILIATION_NEXT_SUMMARY)"
	@printf '\a'
