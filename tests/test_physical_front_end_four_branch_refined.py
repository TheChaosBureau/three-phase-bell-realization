...
from physical_front_end_candidate.experiments.build_four_branch_refined_report import build_physical_front_end_four_branch_refined_report
...
def test_refined_four_branch_fractions_sum_to_one() -> None:
    case = benchmark_refined_four_branch_cases()[0]
    result = simulate_refined_four_branch_candidate(case["state4"], a_deg=case["a_deg"], b_deg=case["b_deg"])
...
def test_refined_candidate_remains_close_to_exact_weights() -> None:
    case = benchmark_refined_four_branch_cases()[2]
    result = simulate_refined_four_branch_candidate(case["state4"], a_deg=case["a_deg"], b_deg=case["b_deg"])
    assert result["metrics"]["rms_error"] < 0.03
    assert result["metrics"]["max_abs_error"] < 0.05


def test_refined_detector_export_remains_frozen_boundary_compatible() -> None:
    case = benchmark_refined_four_branch_cases()[0]
...
def test_refined_candidate_integrated_metrics_and_architecture_hold() -> None:
    case = benchmark_refined_four_branch_cases()[2]
    candidate = simulate_refined_four_branch_candidate(case["state4"], a_deg=case["a_deg"], b_deg=case["b_deg"])
...
def test_refined_candidate_chsh_and_correlator_remain_within_tolerance() -> None:
    exact = {}
    empirical = {}
    rms_errors = []
    for index, (label, (a_deg, b_deg)) in enumerate(DEFAULT_CHSH_SETTINGS.items()):
        candidate = simulate_refined_four_branch_candidate(singlet_state(), a_deg=a_deg, b_deg=b_deg)
...
def test_refined_report_smoke_writes_artifacts_and_chsh_metrics(tmp_path: Path) -> None:
    ...
    outputs = build_physical_front_end_four_branch_refined_report(
        tmp_path / "summary",
    ...
    assert "Refined Four-Branch Front-End Summary" in report_text
